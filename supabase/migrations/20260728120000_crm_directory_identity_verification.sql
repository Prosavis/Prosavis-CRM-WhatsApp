-- Identity fields from verified app users (Firebase verifications.personalInfo)
...
COMMENT ON COLUMN public.crm_directory.document_type IS
  'Identity document type from approved verification (e.g. cedulaCiudadania)';
COMMENT ON COLUMN public.crm_directory.document_number IS
  'Identity document number from approved verification';
COMMENT ON COLUMN public.crm_directory.identity_verified_at IS
  'When identity was verified/merged into directory';
COMMENT ON COLUMN public.crm_directory.identity_source IS
  'Source of identity data (e.g. verification)';

CREATE UNIQUE INDEX IF NOT EXISTS uq_crm_directory_document_number
  ON public.crm_directory (document_number)
  WHERE document_number IS NOT NULL AND trim(document_number) <> '';

-- ---------------------------------------------------------------------------
-- upsert_directory_entry: identity columns + match by app_user_id
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Prefer app_user_id match for verified providers (before phone)
