-- Fase 3 MFL / directorio: lookup global por firebase_crmClient_docId.
-- Antes solo corría dentro del bloque huérfano (sin phone/email), lo que permitía
-- duplicados duplicate_orphan al re-ejecutar backfill legacy con metadata.source_ids.

CREATE INDEX IF NOT EXISTS idx_crm_directory_firebase_crm_client_doc_id
  ON crm_directory ((metadata->'source_ids'->>'firebase_crmClient_docId'))
  WHERE (metadata->'source_ids'->>'firebase_crmClient_docId') IS NOT NULL
    AND length(trim(metadata->'source_ids'->>'firebase_crmClient_docId')) > 0;
...
BEGIN
  ...
  -- Lookup global: legacy Firestore crmClients docId (idempotente en backfill MFL y auto-heal).
