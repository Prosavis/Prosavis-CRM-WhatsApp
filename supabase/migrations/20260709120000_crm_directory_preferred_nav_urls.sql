-- Optional Google Maps / Waze deep links for preferred service location
ALTER TABLE public.crm_directory
  ADD COLUMN IF NOT EXISTS preferred_google_maps_url text,
  ADD COLUMN IF NOT EXISTS preferred_waze_url text;

COMMENT ON COLUMN public.crm_directory.preferred_google_maps_url IS
  'Optional http(s) Google Maps link for preferred service location';
COMMENT ON COLUMN public.crm_directory.preferred_waze_url IS
  'Optional http(s) Waze link for preferred service location';

-- ---------------------------------------------------------------------------
-- upsert_directory_entry: persist preferred Maps/Waze URLs
-- ---------------------------------------------------------------------------
