-- crm_directory: anti-clonado en upsert_directory_entry.
-- Causa raíz de duplicados sin teléfono/email:
--   1. Un "update" desde Firebase (updateDirectoryEntry) reenvía la fila completa
--      pero la función NO leía p_entry->>'id', así que sin phone/email clonaba.
--   2. Filas CRM_CLIENT sin teléfono/email no tenían lookup por identificador de
--      origen (firebase_crmClient_docId) ni por provider_id+service_id.
--
-- Fix: Lookup 0 por id (un update nunca clona) + lookups por identificador de
-- origen cuando no hay phone/email. Misma firma para no romper llamadas.
...
BEGIN
  ...
  -- Lookup 0 (máxima prioridad): id explícito → un update NUNCA clona.
  ...
  -- Lookups por identificador de origen (solo sin phone/email): evitan clones de
  -- filas CRM_CLIENT sin contacto cuando llegan recálculos/citas/pagos.
  IF v_existing_id IS NULL AND v_phone_key IS NULL AND v_email IS NULL THEN
    -- Lookup 6: mismo documento Firestore de origen (firebase_crmClient_docId).
    ...
    -- Lookup 7: mismo provider_id + service_id de un cliente CRM.
