/**
 * Despliega wave A leyendo _callmcp-args-{name}.json.
 * Con SUPABASE_ACCESS_TOKEN (argv[2], env o .env.secrets.local): MCP HTTP.
 * Sin token: imprime instrucciones CallMcpTool y sale 2.
 */
import fs from 'fs';
...
if (!token) {
  console.log(
    JSON.stringify({
      ...
      order: ORDER,
      hint: 'Por cada función: leer scripts/.edge-deploy/_callmcp-args-<name>.json y CallMcpTool deploy_edge_function con el objeto completo.',
    }),
  );
  process.exit(2);
}
...
const client = new Client({ name: 'wave-a-callmcp', version: '1.0.0' }, { capabilities: {} });
...
try {
  ...
  if (parsed.suggestFixed === undefined) {
