/**
 * Prepara y/o despliega wave A.
 * Con token: MCP HTTP deploy_edge_function para cada _callmcp-args-{name}.json
 * Sin token: exit 2 (usar CallMcpTool plugin-supabase-supabase deploy_edge_function)
 */
import fs from 'fs';
...
if (!token) {
  const queue = ORDER.map((name) => ({
    name,
    argsFile: path.join(deployDir, `_callmcp-args-${name}.json`),
  }));
  console.log(
    JSON.stringify({
...
const client = new Client({ name: 'cursor-batch', version: '1.0.0' }, { capabilities: {} });
