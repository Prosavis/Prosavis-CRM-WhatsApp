 ...
 * Token: SUPABASE_ACCESS_TOKEN env o argv[2]
 * Ejecutar desde mcp-runner: node deploy-all-callmcp-from-files.mjs [token]
 */
import fs from 'fs';
...
if (!token) {
  console.error('Token requerido: SUPABASE_ACCESS_TOKEN o argv[2]');
  process.exit(2);
}
...
const client = new Client({ name: 'deploy-all-callmcp-files', version: '1.0.0' }, { capabilities: {} });
const results = [];
