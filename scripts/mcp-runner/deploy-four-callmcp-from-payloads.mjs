/**
 * Lee payloads de scripts/.edge-deploy-payloads/*.json y despliega vía MCP HTTP.
...
async function deployOne(client, args) {
  ...
  if (parsed?.version != null) {
    return { name: args.name, ok: true, version: parsed.version, slug: parsed.slug ?? args.name };
  }
...
const client = new Client({ name: 'deploy-four-payloads', version: '1.0.0' }, { capabilities: {} });
const results = [];
