/**
 * Despliega las 4 funciones desde scripts/.edge-deploy-payloads/*.json
 * vía MCP deploy_edge_function (HTTP OAuth o stdio con token).
 */
import fs from 'fs';
...
async function deployOne(client, args) {
  ...
  if (parsed?.version != null) {
    return { name: args.name, ok: true, version: parsed.version };
  }
  return {
    name: args.name,
    ok: false,
...
const client = new Client(
  { name: 'deploy-four-edge-payloads', version: '1.0.0' },
  { capabilities: {} },
);
const results = [];
...
try {
  ...
  for (const slug of ORDER) {
    ...
    if (!fs.existsSync(src)) {
      results.push({ name: slug, ok: false, error: `No existe ${src}` });
      continue;
    }
    ...
    try {
      ...
      process.stderr.write(
        `${entry.ok ? 'OK' : 'ERR'} ${slug} ${entry.version ?? entry.error}\n`,
      );
    } catch (err) {
