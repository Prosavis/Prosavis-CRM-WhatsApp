/**
 * Despliega una función vía MCP SDK (deploy_edge_function) leyendo JSON payload.
 * Equivalente a CallMcpTool con payload completo desde _mcp-call-{name}.json.
 * Uso: node deploy-one.mjs <function-name> [SUPABASE_ACCESS_TOKEN]
 */
import fs from 'fs';
...
if (!name) {
  console.error('Uso: node deploy-one.mjs <function-name> [token]');
  process.exit(1);
}
if (!token) {
