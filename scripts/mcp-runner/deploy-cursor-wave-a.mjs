/**
 * Despliega wave A desde scripts/.edge-deploy/_cursor-deploy-{name}.json
 * vía Supabase Management API (mismo payload que CallMcpTool deploy_edge_function).
 */
import fs from 'fs';
...
if (!token) {
  console.log(JSON.stringify({ error: 'SUPABASE_ACCESS_TOKEN no configurado' }));
  process.exit(2);
}
