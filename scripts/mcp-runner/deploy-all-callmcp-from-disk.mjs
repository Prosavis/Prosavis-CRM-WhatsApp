/**
 * Lee mcp-invoke-{slug}.json y despliega vía deploy_edge_function (MCP HTTP).
 * Token: argv[2], SUPABASE_ACCESS_TOKEN o búsqueda en Cursor globalStorage.
 */
import fs from 'fs';
...
if (!token) {
  for (const slug of SLUGS) {
    results.push({
      ...
      argsPath: path.join(agentTools, `mcp-invoke-${slug}.json`),
    });
  }
...
const client = new Client({ name: 'deploy-all-callmcp-disk', version: '1.0.0' }, { capabilities: {} });
